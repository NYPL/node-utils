...
class NyplSourceMapper {
  ...
  splitIdentifier(prefixedIdentifier: string): SplitIdentifier {
    ...
    const mapping = Object.keys(nyplSourceMapping)
      .map((nyplSource) =>
        Object.assign({}, { nyplSource }, nyplSourceMapping[nyplSource]),
      )
      .find((properties) => {
        return [
          properties.bibPrefix,
          properties.itemPrefix,
          properties.holdingPrefix,
        ].includes(prefix);
      });
  ...
  prefix(source: string, type: "bib" | "item" | "holding" = "bib"): string {
    ...
    return nyplSourceMapping[source][
      `${type}Prefix` as keyof NyplSourceEntry
    ] as string;
  }
}
...
const createInstance = async (): Promise<NyplSourceMapper> => {
  ...
  const resp = await retryUtils.retry(async () => {
  ...
  // Parse JSON:
  const data = await resp.json().catch((e: Error) => {
